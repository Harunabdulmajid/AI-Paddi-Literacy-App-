  ...
  return (
    <header className="bg-white/90 backdrop-blur-lg sticky top-0 z-20 shadow-sm p-3 md:p-4 border-b border-neutral-200">
      <div className="container mx-auto flex justify-between items-center gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-4 flex-grow justify-end min-w-0">
            
            {/* Scrollable Area for Status Indicators & Language */}
            <div className="flex items-center gap-2 sm:gap-4 overflow-x-auto no-scrollbar mask-fade-right">
                <OfflineIndicator />
            ...
            {/* Fixed Area for Action Buttons (Settings & Profile) - No Overflow */}
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0 ml-1 sm:ml-2 pl-2 border-l border-neutral-200">
                ...
                <div className="relative flex-shrink-0" ref={dropdownRef}>
                    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="rounded-full ring-2 ring-offset-2 ring-transparent hover:ring-primary transition-all block">
                        <UserAvatar name={user.name} avatarId={user.avatarId} avatarUrl={user.avatarUrl} />
                    </button>
                    {isDropdownOpen && (
                        <div className="absolute top-14 right-0 w-64 sm:w-56 bg-white rounded-lg shadow-xl border border-neutral-200 py-2 z-30 animate-slide-up origin-top-right">
