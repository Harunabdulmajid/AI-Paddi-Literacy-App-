                        ...
                        <>
                            ...
                            <form onSubmit={handleForgotPassword} className="space-y-4">
                                <input 
                                    ...
                                    autoFocus
                                />
                                {resetError && <p className="text-red-500 text-sm font-semibold">{resetError}</p>}
                                {resetStatus === 'error' && !resetError && <p className="text-red-500 text-sm">Failed to send reset link. Please try again.</p>}
                            ...
                            <button onClick={() => { setIsForgotPasswordMode(false); setResetStatus('idle'); }} className="mt-6 text-neutral-500 font-bold hover:text-neutral-800 transition">Back to Sign In</button>
                        </>
                    )}
                </div>
            </div>
        ...
        <div className="w-full max-w-md mx-auto">
            <div className="bg-white rounded-2xl shadow-lg p-8 sm:p-10 text-center">
                    ...
                    {authMode === 'signup' && (
                     ...
                     {authMode === 'signin' && (
                         <div className="text-right">
                             <button type="button" onClick={() => { setResetEmail(email); setIsForgotPasswordMode(true); setResetStatus('idle'); setResetError(''); }} className="text-sm font-semibold text-primary hover:underline">Forgot Password?</button>
                         </div>
                     )}
                ...
                <button 
                    ...
                    disabled={isLoading}
                    className="w-full flex items-center justify-center gap-3 bg-white border border-neutral-300 text-neutral-700 font-bold py-3 rounded-lg hover:bg-neutral-50 transition active:scale-95 disabled:bg-neutral-100 disabled:text-neutral-400 shadow-sm"
                >
                    <GoogleIcon />
                    Sign in with Google
                </button>
